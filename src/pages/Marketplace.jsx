    ...
    const loadProducts = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Filter to only display Approved products on marketplace
          const activeProducts = data.filter(p => p.status === 'Approved');
  ...
  return (
    <div className="pt-32 pb-20 bg-background min-h-screen">
      <div className="container mx-auto px-6">
        {/* Back Link */}
        <div className="mb-6">
          <Link to="/" className="inline-flex items-center gap-2 text-slate-500 hover:text-emerald-600 font-bold transition-all text-sm">
            ← Back to Home
          </Link>
        </div>
        {/* Header */}
